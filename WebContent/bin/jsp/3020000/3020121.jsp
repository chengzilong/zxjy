...
$(document).ready(function(){
   ...
   mmg = $('.mmg').mmGrid({
        ...
        ,method: 'post'
        ,params:{CMD : "<%=Servlet3020120.CMD_SELECT_COURSE%>"}
        ,remoteSort:true
        ,sortName: 'KCFY_FYID'
        ,sortStatus: 'ASC'
        ,root: 'items'
  ...
  //保存点击事件（新增/更新）
	...
	$('#selectEditTjjs').attr("disabled", "disabled");
	/* 初始化数据 */
	if(optionFlag == "UpdTk"){
...
//查询bean
function makeBeanInSearch(strSEARCH_JSBM,strJSXM){
	this.SEARCH_JSBM = strSEARCH_JSBM;
	this.JSXM = strJSXM;
}
//查询数据
...
function makeBeanInEdit(strBMXX_BMID,strBMXX_XSID,strBMXX_XXID,strBMXX_SDID,strBMXX_XS,
		strBMXX_JSID,strBMXX_FY,strBMXX_LXID,strBMXX_KCFYID,strBMXX_TJJSID,strBMXX_BZXX,strBMXX_SSBC,strSELECT_JSBM){
    ...
    this.SELECT_JSBM = strSELECT_JSBM;
}
//新增数据
function insertData(dataBean){
	...
	$.ajax({
		...
		url: "<%=basePath%>/Servlet3020120",
		data: {
			CMD    : "<%=Servlet3020120.CMD_INSERT_XK%>",
...
function updateData(dataBean){
    ...
    $.ajax({
      ...
      url: "<%=basePath%>/Servlet3020120",
      data: {
         CMD    : "<%=Servlet3020120.CMD_UPDATE_TK%>",
         BeanIn : JSON.stringify(beanIn)
      },
      complete :function(response){},
      success: function(response){
	...
	<fieldset id = "selectRegion">
		...
		<table>
			<tr>
				...
				<td>
					<input id="selectJsdh" name="教师电话" maxlength="20" />
				</td>
				<th style="width:100px">教师姓名</th>
				<td>
					<input id="selectJsxm" name="教师姓名" maxlength="20" />
				</td>
