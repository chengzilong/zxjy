...
$(document).ready(function(){
   ...
   mmg = $('.mmg').mmGrid({
        ...
        ,method: 'post'
        ,params:{CMD : "<%=Servlet3020120.CMD_SELECT_CLASS%>"}
        ,remoteSort:true
        ,sortName: 'BCXX_BCID'
        ,sortStatus: 'ASC'
        ,root: 'items'
...
//查询bean
function makeBeanInSearch(strBCXX_BCMC, strBCXX_KCID, strBCXX_BCZT, strSEARCH_JSBM, strJSXM){
...
function makeBeanInEdit(strBMXX_BMID,strBMXX_XSID,strBMXX_XXID,strBMXX_SDID,strBMXX_XS,
		...
		strBMXX_SSBC,strYBCID,strSELECT_JSBM){
...
function checkBcxs(dataBean) {
	...
	$.ajax({
		...
		url: "<%=basePath%>/Servlet3020120",
		data: {
			CMD		: "<%=Servlet3020120.CMD_CHK_EXIST%>",
...
function insertData(dataBean){
	...
	$.ajax({
		...
		url: "<%=basePath%>/Servlet3020120",
		data: {
			CMD    : "<%=Servlet3020120.CMD_INSERT_XB%>",
...
function updateData(dataBean){
    ...
    $.ajax({
      ...
      url: "<%=basePath%>/Servlet3020120",
      data: {
         CMD    : "<%=Servlet3020120.CMD_UPDATE_TB%>",
         BeanIn : JSON.stringify(beanIn)
      },
      complete :function(response){},
      success: function(response){
