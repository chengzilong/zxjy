...
$(document).ready(function(){
   ...
   mmg = $('.mmg').mmGrid({
        ...
        ,method: 'post'
        ,params:{CMD : "<%=Servlet3030120.CMD_SELECT%>"}
        ,remoteSort:true
        ,sortName: 'BCXX_BCID'
        ,sortStatus: 'ASC'
        ,root: 'items'
   ...
   /* 页面初始化加载数据 */
    ...
    $('#btnDel').on('click', function(){
 	   ...
 	   if(arrList[0].SFYZ=="已验证"){
 	   	  layer.alert('此班次已经通过验证，无法删除！', 0, '友情提示');
 	   	  return;
 	   }
...
function btn_Add() {
	obj = new Object();
	obj.option = "Add";
	iframeLayerOpen('<%=basePath%>/bin/jsp/3030000/3030121.jsp');
}
//调整班次方法
function btn_Upd() {
	var arrList = mmg.selectedRows();
   ...
   if(arrList[0].SFYZ=="已验证"){
	   layer.alert('友情提示：此班次已经通过验证，无法修改！', 0, '友情提示');
	   return;
   }
	obj = new Object();
	obj.option = "Upd";
	...
	obj.fyid = arrList[0].BCXX_KCFYID;
	iframeLayerOpen('<%=basePath%>/bin/jsp/3030000/3030121.jsp');
}
//删除班次方法
