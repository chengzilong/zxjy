...
$(document).ready(function(){
   ...
   mmg = $('.mmg').mmGrid({
        ...
        ,method: 'post'
        ,params:{CMD : "<%=Servlet3030140.CMD_SELECT_ENROLL%>"}
        ,remoteSort:true
        ,sortName: 'BMXX_BMID'
        ,sortStatus: 'ASC'
        ,root: 'items'
...
function checkExist() {
	...
	$.ajax({
		...
		url: "<%=basePath%>/Servlet3030140",
		data: {
			CMD		: "<%=Servlet3030140.CMD_CHK_EXIST%>",
			BCID			: obj.bcId,
			STUIDS		: stuIds,
			STUMCS      :stuMcs
		},
...
function relationData() {
	...
	$.ajax({
		...
		url: "<%=basePath%>/Servlet3030140",
		data: {
			CMD		: "<%=Servlet3030140.CMD_RELATION%>",
			BCID			: obj.bcId,
			DATAIDS	: dataIds,
			STUIDS		: stuIds
		},
		...
		success : function(response) {
			var CMD = response[0];
			if (CMD == "CMD_OK") {
				layer.msg('恭喜：保存数据成功！', 1, 9);
				blnRet = true;
			} else if(CMD == "CMD_ERROR"){
				layer.msg('对不起：保存数据失败！', 1, 8);
				blnRet = false;
			} else if (CMD == "CMD_EXCEPTION") {
				layer.msg('提示：保存数据出错！', 1, 0);
				blnRet = false;
			}
		}
	});
