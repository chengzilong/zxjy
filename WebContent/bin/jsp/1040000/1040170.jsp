<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@ page import="com.xsjy.servlet.servlet1040000.Servlet1040170"%> 
<%
...
$(document).ready(function(){
   var cols = [
       ...
       { title:'课程类型', name:'LXMC' ,width:100, sortable:true, align:'center',lockDisplay: true },
       { title:'上课时段', name:'SDMC' ,width:80, sortable:true, align:'center',lockDisplay: true  },
       { title:'学时', name:'KCFY_XS' ,width:50, sortable:true, align:'center',lockDisplay: true  },
       { title:'教师姓名', name:'JSMC' ,width:100, sortable:true, align:'center',lockDisplay: true  },
       { title:'费用', name:'KCFY_FY' ,width:80, sortable:true, align:'center',lockDisplay: true  }
   ];
   ...
   mmg = $('.mmg').mmGrid({
         height: intheight
        ,cols: cols
        ,url: '<%=basePath%>/Servlet1040170'
        ,method: 'post'
        ,params:{CMD : "<%=Servlet1040170.CMD_SELECT%>"}
        ,remoteSort:true
        ,sortName: 'KCFY_FYID'
        ,sortStatus: 'ASC'
        ,root: 'items'
   ...
   mmg.on('rowSelected', function(e, item, rowIndex, colIndex){//选择数据行
       ...
       $("#selectEditJS  option[value='"+arrList.KCFY_JSID+"'] ").attr("selected",true);
	   ...
	   if(this.checked == true){
		   ...
		   if(arrList.length>0){
	         ...
	         $("#selectEditJS  option[value='"+arrList[0].KCFY_JSID+"'] ").attr("selected",true);
	         ...
	         setButtonStatus("4");   
		       ...
		       $("#selectEditJS  option[value='000'] ").attr("selected",true);
		       $('#txtEditXS').val("");
		       $('#txtEditFY').val("");
		       $('#txtFY').val("");
   ...
   $('#btnDel').on('click', function(){
    ...
    loadEditSelect($("#selectEditJS"),"TYPE_JSXM","教师");
    ...
    loadSearchSelect($("#txtSelJS"),"TYPE_JSXM","教师");
...
function makeBeanIn(strKCMC,strKCLX,strSKSD,strJS){
	this.KCFY_XXID = strKCMC;
	this.KCFY_LXID = strKCLX;
    this.KCFY_SDID = strSKSD;
    this.KCFY_JSID = strJS;
}
function makeBeanInadd(strKCMC,strKCLX,strSKSD,strXS,strJS,strFY){
	this.KCFY_XXID = strKCMC;
    this.KCFY_LXID = strKCLX;  
    this.KCFY_SDID = strSKSD;  
    this.KCFY_XS = strXS;
...
function makeBeanInedit(strFYID,strKCMC,strKCLX,strSKSD,strXS,strJS,strFY){
	...
	$.ajax({
		...
		success: function(response){
			...
			if(strResault=="SUCCESS"){
				layer.msg('恭喜：新增课程费用信息成功！', 1, 9);
				blnRet = true;
			}else if(strResault=="FAILURE"){
				layer.msg('对不起：新增课程费用信息失败！', 1, 8);
				blnRet = false;
			}else{
				layer.msg('提示：新增课程费用信息出错！', 1, 0);
				blnRet = false;
			}
		}
	});
...
function updateKCFYCode(){
    ...
    $.ajax({
      ...
      success: function(response){
          var strResault = response[0];
          if(strResault=="SUCCESS"){
        	  layer.msg('恭喜：修改课程费用信息成功！', 1, 9);
             blnRet = true;
          }else if(strResault=="FAILURE"){
        	  layer.msg('对不起：修改课程费用信息失败！', 1, 8);
             blnRet = false;
          }else{
        	  layer.msg('提示：修改课程费用信息出错！', 1, 0);
             blnRet = false;
          }
      }
   });
...
function deleteKCFYCode(){
	...
	$.ajax({
		...
		success : function(response) {
			var strResault = response[0];
			if (strResault == "SUCCESS") {
				layer.msg('恭喜：删除课程费用信息成功！', 1, 9);
				blnRet = true;
			} else if(strResault == "FAILURE") {
				layer.msg('对不起：删除课程费用信息失败！', 1, 8);
				blnRet = false;
			} else{
        	 	layer.msg('提示：删除课程费用信息出错！', 1, 0);
                blnRet = false;
          }
		}
	});
...
function setButtonStatus(strFlag) {
	if (strFlag == "1") {//初始状态
		...
		$('#selectEditJS').attr("disabled","disabled");
		...
		if (strFlag == "31") {//新增
			...
			$('#selectEditJS').removeAttr("disabled");
...
function funEditCheck() {
	if (optionFlag == "Add" || optionFlag == "Upd") {
		...
		if ($('#selectEditJS').val() == "000") {
			layer.alert('请选择教师！', 0, '友情提示', function() {
				$('#selectEditJS').focus();
				layer.close(layer.index);
			});
			return false;
