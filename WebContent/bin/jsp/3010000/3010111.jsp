...
<%
	...
	//out.write( new ActionEnter( request, basePath ).exec() );
%>      
...
<!--富文本编辑器Start  -->
<script type="text/javascript" src="<%=basePath%>/bin/js/ueditor/ueditor.config.js"></script>
<script type="text/javascript" src="<%=basePath%>/bin/js/ueditor/ueditor.all.min.js"></script>
<!--富文本编辑器End  -->
...
$(document).ready(function(){
   ...
   mmg = $('.mmg').mmGrid({
        ...
        ,method: 'post'
        ,params:{CMD : "<%=Servlet3010110.CMD_SELECTHD%>"}
        ,remoteSort:true
        ,sortName: 'HDXX_HDID'
        ,sortStatus: 'ASC'
        ,root: 'items'
    ...
    mmg.on('cellSelected', function(e, item, rowIndex, colIndex){
	  var arrList = mmg.row(rowIndex);
        ...
        if($(e.target).is('.img-del')){
            ...
            if(confirm("是否删除该条回答信息？")==false) return;
   ...
   mmg.on('rowSelected', function(e, item, rowIndex, colIndex){//选择数据行
    ...
    $('#btnReturn').on('click', function(){
	  window.location.href='<%=basePath%>/bin/jsp/3010000/3010110.jsp';
   });
	...
	<fieldset id = "editRegion">
		...
		<table id="detailCanvas" class="eTable6" >
			...
			<tr>
				...
				<script id="txtEditHDNR" type="text/plain"  style="width:700px;height:150px;"></script>	
...
<script type="text/javascript">
	    ...
	    //editor.render("txtEditHDNR");
		//1.2.4以后可以使用一下代码实例化编辑器
		//UEDITOR_CONFIG.UEDITOR_HOME_URL = '<%=basePath%>/bin/ueditor/';
		UE.getEditor('txtEditHDNR');
</script>
</html>