<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@ page import="com.xsjy.servlet.servlet3010000.Servlet3010120"%> 
<%
...
$(document).ready(function(){
   ...
   mmg = $('.mmg').mmGrid({
         height: intheight
        ,cols: cols
        ,url: '<%=basePath%>/Servlet3010120'
        ,method: 'post'
        ,params:{CMD : "<%=Servlet3010120.CMD_SELECT%>"}
        ,remoteSort:true
        ,sortName: 'XXXX_XXID'
        ,sortStatus: 'ASC'
        ,root: 'items'
   ...
   mmg.on('cellSelected', function(e, item, rowIndex, colIndex){
	  var arrList = mmg.row(rowIndex);
        if($(e.target).is('.img-info')){
			...
			dataBean = item;
     		iframeLayerOpenown('<%=basePath%>/bin/jsp/3010000/3010121.jsp',item.MXID);	
        }
   });
...
function iframeLayerOpenown(url,mxid) {
	$.layer({
	      ...
	      iframe: {src: url},
		       ...
		       updateCKZT(mxid);
			   window.parent.setMessCount('3010120');
			   loadGridByBean();	
	      }
	  });
}
	...
	<fieldset id = "selectRegion">
		...
		<table>
			<tr>
		        ...
		        <td><input  id="txtSelFBSJ_BEGIN"  style="font-size:12px;line-height:16px; height:18px;"  class="laydate-icon-default"  type="text"  onclick="laydate()" readonly></td>
		        <th >-</th>
		        <td><input  id="txtSelFBSJ_END"  style="font-size:12px;line-height:16px; height:18px;"  class="laydate-icon-default"  type="text"  onclick="laydate()" readonly></td>
