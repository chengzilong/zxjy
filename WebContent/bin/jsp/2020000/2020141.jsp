...
$(document).ready(function(){
   ...
   var cols = [
       ...
       { title:'评价内容', name:'PJXX_PJNR' ,width:280, sortable:true, align:'center',lockDisplay: true  },
   ...
   mmg = $('.mmg').mmGrid({
        ...
        ,method: 'post'
        ,params:{CMD : "<%=Servlet2020140.CMD_TALK_SELECT%>"}
        ,remoteSort:true
        ,sortName: 'BCXX_BCID'
        ,sortStatus: 'ASC'
        ,root: 'items'
...
//定义bean
function makeBeanIn(strPJXX_PJID,strPJXX_PJJF,strPJXX_PJNR,strJSBM,strBCXX_BCID){
...
function insertData(bcId){
	...
	$.ajax({
		...
		url: "<%=basePath%>/Servlet2020140",
		data: {
			CMD    : "<%=Servlet2020140.CMD_TALK_INSERT%>",
		...
		success: function(response){
			...
			if (strResult == "SUCCESS") {
				layer.msg('恭喜：新增评价成功！', 1, 9);
				blnRet = true;
			} else if (strResult == "FAILURE") {
				layer.msg('对不起：新增评价失败！', 1, 8);
				blnRet = false;
			} else {
				layer.msg('提示：新增评价出错！', 1, 0);
				blnRet = false;
			}
		}
	});
...
function updateData(pjId){
    ...
    $.ajax({
      ...
      url: "<%=basePath%>/Servlet2020140",
      data: {
         CMD    : "<%=Servlet2020140.CMD_TALK_UPDATE%>",
         BeanIn : JSON.stringify(beanIn)
      },
      complete :function(response){},
      success: function(response){
          var strResult = response[0];
          if (strResult == "SUCCESS") {
        	  layer.msg('恭喜：修改评价成功！', 1, 9);
             blnRet = true;
          } else if (strResult == "FAILURE") {
             layer.msg('对不起：修改评价失败！', 1, 8);
             blnRet = false;
          } else {
        	  layer.msg('提示：修改评价出错！', 1, 0);
             blnRet = false;
          }
      }
   });
...
function deleteData(pjId){
	...
	$.ajax({
		...
		url: "<%=basePath%>/Servlet2020140",
		data: {
			CMD    : "<%=Servlet2020140.CMD_TALK_DELETE%>",
			BeanIn : JSON.stringify(beanIn)
		},
		complete : function(response) {},
		success : function(response) {
			var strResult = response[0];
			if (strResult == "SUCCESS") {
				layer.msg('恭喜：删除评价成功！', 1, 9);
				blnRet = true;
			} else if (strResult == "FAILURE") {
				layer.msg('对不起：删除评价失败！', 1, 8);
				blnRet = false;
			} else {
	        	layer.msg('提示：删除评价出错！', 1, 0);
	            blnRet = false;
	        }
		}
	});
