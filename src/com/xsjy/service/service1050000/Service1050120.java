...
public class Service1050120 extends BaseService {
	 ...
	 */
	public int getDataCount(Pojo1050120 beanIn) throws Exception {
		int result = 0;
	 ...
	 */
	public List<Pojo1050120> getDataList(Pojo1050120 beanIn, int page,
	 ...
	 * @return int
	 * @author ztz
	 * @date 2014年12月15日 下午4:21:43
	 */
	public int getCourseCount(Pojo1050121 beanIn) throws Exception {
		int result = 0;
	 ...
	 * @return
	 * @throws Exception
	 * @return List<Pojo1050121>
	 * @author ztz
	 * @date 2014年12月15日 下午4:22:16
	 */
	public List<Pojo1050121> getCourseList(Pojo1050121 beanIn, int page,
			int limit, String sort) throws Exception {
		...
		try {
			...
			StringBuffer execSql = this.getPageSqL(strbuf.toString(), page, limit,
					sort);
			ResultSetHandler<List<Pojo1050121>> rs = new BeanListHandler<Pojo1050121>(
					Pojo1050121.class);
			result = db.queryForBeanListHandler(execSql, rs);
	 ...
	 * @return String
	 * @author ztz
	 * @date 2014年12月15日 下午4:24:35
	 */
	private String searchCourseSql(Pojo1050121 beanIn) throws Exception {
		StringBuffer strbuf = new StringBuffer();
		...
		try {
			...
			/* 课程名称 */
			if (MyStringUtils.isNotBlank(beanIn.getKCFY_XXID())) {
				if (!"000".equals(beanIn.getKCFY_XXID())) {
					strbuf.append(" AND A.KCFY_XXID = '").append(beanIn.getKCFY_XXID()).append("'");
				}
			}
			/* 课程类型 */
			if (MyStringUtils.isNotBlank(beanIn.getKCFY_LXID())) {
				if (!"000".equals(beanIn.getKCFY_LXID())) {
					strbuf.append(" AND A.KCFY_LXID = '").append(beanIn.getKCFY_LXID()).append("'");
				}
			}
			/* 上课时段 */
			if (MyStringUtils.isNotBlank(beanIn.getKCFY_SDID())) {
				if (!"000".equals(beanIn.getKCFY_SDID())) {
					strbuf.append(" AND A.KCFY_SDID = '").append(beanIn.getKCFY_SDID()).append("'");
				}
			}
			/* 授课教师 */
			if (MyStringUtils.isNotBlank(beanIn.getKCFY_JSID())) {
				if (!"000".equals(beanIn.getKCFY_JSID())) {
					strbuf.append(" AND A.KCFY_JSID = '").append(beanIn.getKCFY_JSID()).append("'");
				}
			}
		} catch (Exception e) {
			MyLogger.error(this.getClass().getName(), e);
	 ...
	 * @return int
	 * @author ztz
	 * @date 2014年12月17日 上午11:53:50
	 */
	public int getClassCount(Pojo1050122 beanIn) throws Exception {
		int result = 0;
	 ...
	 * @return
	 * @throws Exception
	 * @return List<Pojo1050122>
	 * @author ztz
	 * @date 2014年12月17日 上午11:53:59
	 */
	public List<Pojo1050122> getClassList(Pojo1050122 beanIn, int page,
			int limit, String sort) throws Exception {
		List<Pojo1050122> result = null;
		...
		try {
			...
			StringBuffer execSql = this.getPageSqL(strbuf.toString(), page, limit,
					sort);
			ResultSetHandler<List<Pojo1050122>> rs = new BeanListHandler<Pojo1050122>(
					Pojo1050122.class);
			result = db.queryForBeanListHandler(execSql, rs);
	 ...
	 * @return String
	 * @author ztz
	 * @date 2014年12月17日 上午11:54:10
	 */
	private String searchClassSql(Pojo1050122 beanIn) throws Exception {
		StringBuffer strbuf = new StringBuffer();
	 ...
	 */
	public boolean insertData(Pojo1050120 beanIn) throws Exception {
		boolean result = false;
		int countBMXX = 0;
		int countBCXS = 0;
		...
		try {
			...
			if ("000".equals(beanIn.getBMXX_TJJSID())) {
				...
				strbufBMXX.append("     '1', ");//报名方式
			} else {
				strbufBMXX.append("     '"+beanIn.getBMXX_TJJSID()+"', ");//推荐教师ID
				strbufBMXX.append("     '2', ");//报名方式
			}
	 ...
	 * @return boolean
	 * @author ztz
	 * @date 2014年12月16日 下午3:35:04
	 */
	public boolean updateData(Pojo1050120 beanIn) throws Exception {
		boolean result = false;
		int countBMXX = 0;
		int countBCXS = 0;
