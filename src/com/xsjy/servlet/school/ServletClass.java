...
import com.xsjy.service.school.ServiceClass;

/**
 * Servlet implementation class ServletLession
 */
@WebServlet("/ServletClass")
public class ServletClass extends BaseServlet {
	...
	private void getDataCount(Map<String, String[]> inputdata) throws Exception {
		PojoClass dataBean = (PojoClass) this.getObject(inputdata, "BeanIn", PojoClass.class);
		int dataCount = 0;
	...
	private void getDataList(Map<String, String[]> inputdata) throws Exception {
		...
		PojoClass beanIn = (PojoClass) this.getObject(inputdata,
				"BeanIn", PojoClass.class);
		Pojo_YHXX beanUser = (Pojo_YHXX)getSessionObject(SessionAttribute.LOGIN_USER);

		List<PojoClass> dataList = new ArrayList<PojoClass>();
	 ...
	 * @return void
	 * @author ztz
	 * @date 2015年1月5日 下午2:49:57
	 */
	private void checkRoleAndData(Map<String, String[]> inputdata) throws Exception {
		String classId = this.getString(inputdata, "classID");//班次ID

		try {
			if (!"106".equals(beanUser.getYHXX_JSID())) {
				arrResult.add("ROLE_ERROR");
			} else {
				boolean isDataExist = service.isDataExist(classId, beanUser.getYHXX_YHID());
				if (isDataExist) {
	...
	private void insertData(Map<String, String[]> inputdata) throws Exception {
		...
		try {
			result = service.insertData(classId, beanUser);
			if (result) {
				arrResult.add("SUCCESS");
			} else {
